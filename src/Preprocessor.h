...
public:
    ...
    /* Expands a DEFINE instruction that defines a variable RESULT as the reduction of the given vector VEC.
     * The reduction cascadingly applies the given FUNC to the components of the vector.
     * For example, "define z = reduce_vector v add" returns the sum of the components in the vector V.
     *
     ...
     */
    int expand_reduce_vector_instruction(const string& result, const string& vec,
     ...
     * A variable can be defined in one of six ways:
     *
     ...
     * 
     * 5. "define <var_name> = reduce_vector <vector operand> <binary primitive/binary macro"
     *
     * 6. "define <var_name> = <another variable with a different name>"
     *
    ...
    /* Returns true if the given NAME is a component of a vector that has been previously declared, and false otherwise.
     *  
     * For example: 
     *  declare_vector input x 3  
     *  is_vector_component(x.0) == true
     *  is_vector_component(x1) == false
     *  is_vector_component(y.0) == false
     */
    bool is_vector_component(const string& name);

    /* Returns true if the vector by the given NAME has had any of its components defined.
     * Recall that a vector can be defined using the DEFINE_VECTOR instruction, or its components can be individually defined.
     * The DEFINE_VECTOR instruction defines the vector and all of its components.
     *
     * Returns false if the vector by the given NAME has not yet been declared,
     *  or if it's been declared, but none of its components have been defined.
     */
    bool has_defined_components(const string& name);
 
    /* Returns true if the vector by the given NAME has had all of its components defined.
     * This method returns true if the vector was defined using a DEFINE_VECTOR instruction,
     *  OR if each of the components was individually defined previously.
     *  Recall that the DEFINE_VECTOR instruction marks all of the components as defined.
     *
     * Returns false if the vector by the given NAME has not yet been declared,
     *  or if any of its components have not been marked as defined.
     */
    bool all_components_defined(const string& name);

    /* Returns the name of the COMPONENT_NUMth component of the vector with the given VEC_NAME.
     * ex. get_vector_component_name("my_vec", 3) = "my_vec.3"
     */
    string get_vector_component_name(const string& vec_name, int component_num);

    /* Returns the name of the INTVAR_NUMth intermediate variable of the VAR_NAME.
     * ex. get_intermediate_name("my_var", 3) = "my_var.3"
     */
    string get_intermediate_name(const string& var_name, int intvar_num);
