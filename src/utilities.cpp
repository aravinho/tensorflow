...
OperationType get_operation_type(const string &oper) {
    ...
    if (oper.compare("reduce_vector") == 0) {
        return OperationType::REDUCE_VECTOR;
    }
...
string get_operation_name(const OperationType oper) {
    if (oper == OperationType::ADD) {
        return "add";
    }
    if (oper == OperationType::SUB) {
        return "sub";
    }
    if (oper == OperationType::MUL) {
        return "mul";
    }
    if (oper == OperationType::DOT) {
        return "dot";
    }
    if (oper == OperationType::LOGISTIC) {
        return "logistic";
    } 
    if (oper == OperationType::EXP) {
        return "exp";
    }
    if (oper == OperationType::POW) {
        return "pow";
    }
    if (oper == OperationType::LN) {
        return "ln";
    }
    if (oper == OperationType::SCALE_VECTOR) {
        return "scale_vector";
    }
    if (oper == OperationType::INCREMENT_VECTOR) {
        return "increment_vector";
    } 
    if (oper == OperationType::COMPONENT_WISE_ADD) {
        return "component_wise_add";
    }
    if (oper == OperationType::COMPONENT_WISE_MUL) {
        return "component_wise_mul";
    }
...
bool is_valid_primitive(const string& name) {
    ...
    return oper_type == OperationType::ADD || oper_type == OperationType::SUB || oper_type == OperationType::MUL || oper_type == OperationType::LOGISTIC || 
...
bool is_binary_primitive(const string& name) {
    OperationType oper_type = get_operation_type(name);
    return oper_type == OperationType::ADD || oper_type == OperationType::SUB || oper_type == OperationType::MUL || oper_type == OperationType::POW;
}
...
bool is_reduce_vector(const string& name) {
    OperationType oper_type = get_operation_type(name);
    return oper_type == OperationType::REDUCE_VECTOR;
}
...
string get_error_message(int error_code) {
    if (error_code > -1 || error_code < (-1 * NUM_ERROR_TYPES)) {
        return "";
    }
    return error_messages[(error_code * -1) - 1];
}
...
int tokenize_line(const string& line, vector<string> *tokens, const string& delimiters) {
    ...
    while (curr_delim_pos != string::npos) {
        ...
        // it is possible, though that curr_delim_pos is 0, and next_delim_pos is 1
        // in this case, we must grab that one-character token
        if (next_delim_pos > curr_delim_pos + 1 || curr_delim_pos == 0) {
